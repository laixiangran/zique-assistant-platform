...
export default function PackagesPage() {
  ...
  const fetchPackages = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const enabledPackages = data.data.filter(
          (pkg: Package) => pkg.enabled
        ).length;
  ...
  const fetchUserPackages = async () => {
    try {
      ...
      if (data.success) {
        ...
        const activeOrders = data.data.userPackages.filter(
          (order: UserPackage) => order.status === 'active'
        ).length;
  ...
  const handleOrderPackage = async (values: any) => {
    try {
      const response = await userPackagesAPI.orderPackage({
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      <Modal
        ...
        width={600}
      >
        <Form form={form} layout='vertical' onFinish={handleSubmitPackage}>
          <Form.Item
            label='套餐名称'
            name='name'
            rules={[
          ...
          <Form.Item label='套餐特性（每行一个）' name='features'>
            <Input.TextArea
              rows={4}
          ...
          <>
            ...
            >
              ...
              <Descriptions.Item label='时长'>
                {selectedPackage.duration_months} 个月
              </Descriptions.Item>
              <Descriptions.Item label='店铺限制'>
                {selectedPackage.mall_limit} 个店铺
              </Descriptions.Item>
