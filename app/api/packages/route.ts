...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(errorResponse('获取套餐列表失败，请稍后重试'), { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || price === undefined || !duration_months || !max_shop_count) {
      return NextResponse.json(errorResponse('请填写完整的套餐信息'), { status: 400 })
    }

    // 验证数据类型
    if (typeof price !== 'number' || price < 0) {
      return NextResponse.json(errorResponse('价格必须为非负数'), { status: 400 })
    }

    if (typeof duration_months !== 'number' || duration_months <= 0) {
      return NextResponse.json(errorResponse('时长必须为正数'), { status: 400 })
    }

    if (typeof max_shop_count !== 'number' || max_shop_count <= 0) {
      return NextResponse.json(errorResponse('店铺数量必须为正数'), { status: 400 })
    }
    ...
    return NextResponse.json(errorResponse('创建套餐失败，请稍后重试'), { status: 500 })
  }
}