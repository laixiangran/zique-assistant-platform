...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(errorResponse('获取用户套餐记录失败，请稍后重试'), { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    endTime.setMonth(endTime.getMonth() + (membershipPackage as any).duration_months)
    ...
    return NextResponse.json(errorResponse('订购套餐失败，请稍后重试'), { status: 500 })
  }
}