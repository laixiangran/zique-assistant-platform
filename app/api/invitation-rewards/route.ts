...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      errorResponse('获取邀请奖励记录失败，请稍后重试'),
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!reward) {
      return NextResponse.json(errorResponse('奖励不存在或已领取'), {
        status: 404,
      });
    }
    ...
    await UserOperationLog.create({
      ...
      request_data: JSON.stringify({
        reward_id,
        reward_type: (reward as any).reward_type,
    ...
    return NextResponse.json(errorResponse('领取邀请奖励失败，请稍后重试'), {
      status: 500,
    });
  }
}
