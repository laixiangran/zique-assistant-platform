...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validPlatforms.includes(platform)) {
      return NextResponse.json(errorResponse('不支持的平台类型'), {
        status: 400,
      });
    }
    ...
    return NextResponse.json(
      successResponse(formatObjectDates(shopBinding), '店铺绑定成功')
    );
  } catch (error) {
