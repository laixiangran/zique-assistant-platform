...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (decoded.type !== 'user') {
      return NextResponse.json(errorResponse('只有主账户可以查看子账户详情'), { status: 403 })
    }
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // 只有主账户可以更新子账户
    if (decoded.type !== 'user') {
      return NextResponse.json(errorResponse('只有主账户可以更新子账户'), { status: 403 })
    }
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // 只有主账户可以删除子账户
    if (decoded.type !== 'user') {
      return NextResponse.json(errorResponse('只有主账户可以删除子账户'), { status: 403 })
    }
    ...
    return NextResponse.json(errorResponse('删除子账户失败，请稍后重试'), { status: 500 })
  }
}